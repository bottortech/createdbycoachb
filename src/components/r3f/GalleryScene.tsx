...
import { BOOKING_PAYMENT_URL, LINKEDIN_URL, GITHUB_URL, ENTERPRISE_PDF_URL } from "@/lib/links";
  ...
  return (
    <div className="fixed inset-0">
      ...
      <GalleryOverlayPanel open={activePanel === "enterprise"} onClose={() => setActivePanel(null)} label="Enterprise Hall" title="Bottor Technologies Inc.">
        <p className="text-sm text-gallery-muted leading-relaxed mb-6">{ENTERPRISE_DESCRIPTION}</p>
        <div className="grid gap-3 sm:grid-cols-2">{ENTERPRISE_SERVICES.map((c) => (<div key={c} className="rounded-lg border border-white/[0.06] bg-white/[0.02] px-4 py-3"><p className="text-xs font-medium text-gallery-white">{c}</p></div>))}</div>
        <div className="mt-6"><a href={ENTERPRISE_PDF_URL} target="_blank" rel="noopener noreferrer" className="rounded-full border border-gallery-gray px-5 py-2 text-xs font-medium text-gallery-light hover:border-gallery-accent hover:text-gallery-accent transition-colors">Download PDF</a></div>
      </GalleryOverlayPanel>
